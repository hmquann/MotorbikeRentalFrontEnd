...
const checkTimeDifference = (newDate, startDate, receiveTime) => {
  const currentDate = new Date();
  const startDateTime = parseDateTime(startDate, receiveTime);
  const timeDifference = (newDate - startDateTime) / (1000 * 60 * 60); // hours
  return timeDifference >= 2; // Must be at least 6 hours
};
...
const MotorbikeSchedulePopUp = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = () => {
    ...
    if (!checkTimeDifference(endDateTime, startDate, receiveTime)) {
      setTimeError("Thời gian nhận và trả xe phải ít nhất 2 giờ");
      return;
    }
  ...
  return (
    <div className={sharedClasses.overlay}>
      <div
        className={`${sharedClasses.popupContainer} flex flex-col justify-center items-center p-4 max-w-sm mx-auto`}
      >
        <button
          className={`${sharedClasses.closeButton} self-end`}
          onClick={onClose}
        >
          <svg
        ...
        <h2
          className={`text-center ${sharedClasses.textLg} ${sharedClasses.fontSemiBold} mb-2 text-black`}
        >
        ...
        <div className="border-t border-gray-300 pt-2 w-full bg-white-100">
          <div className="text-center">
            <p className="text-black text-sm">
              Thời gian thuê: {receiveTime}, {startDate.toLocaleDateString()} -{" "}
              {returnTime}, {endDate ? endDate.toLocaleDateString() : ""}
            </p>
          </div>
          <div className="text-center mt-1">
            <p className="text-sm text-gray-500">
              {endDate
                ? `Số ngày thuê: ${calculateDaysDifference(
                    startDate,
                    receiveTime,
                    endDate,
                    returnTime
        ...
        <button
          className={`w-6/12 mt-2 hover:bg-green-600 transition-shadow hover:shadow-2xl ${sharedClasses.green500} ${sharedClasses.white} ${sharedClasses.p2} ${sharedClasses.rounded}`}
