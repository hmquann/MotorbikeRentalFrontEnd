...
const Register = () => {
  ...
  const validatePassword = (password) => {
    return (!(password.length < 8 || password.length > 20 || !/[A-Z]/.test(password) || !/\d/.test(password)));
  }
  ...
  const handleChange = (e) => {
    ...
    if (name === 'password') {
      if (!validatePassword(value)) {
        setPasswordError('Password length 8-20 characters and contains upper character and number.');
      } else {
        setPasswordError('');
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (firstNameError || lastNameError || phoneError || emailError || repasswordError || passwordError) {
    ...
    navigate('/emailVerify',{ state: formData });
