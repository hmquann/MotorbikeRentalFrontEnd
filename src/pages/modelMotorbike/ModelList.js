...
const ModelList = () => {
  ...
  const fetchModels = async () => {
    try {
      ...
      const response = await axios.get(
        `https://rentalmotorbikebe.azurewebsites.net/api/model/getAllModel/${currentPage}/${pageSize}`
      );
  ...
  const searchModels = async (searchTerm, page, size) => {
    try {
      setIsLoading(true)
      const response = await axios.get(`https://rentalmotorbikebe.azurewebsites.net/api/model/search`, {
        params: {
          searchTerm,
          page,
          size
