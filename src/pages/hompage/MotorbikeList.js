        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {motorbikeList.length === 0 ? (
            ...
            motorbikeList.map((motorbike) => (
              <div
  key={motorbike.id}
  className="relative p-3 bg-white rounded-lg shadow-md dark:bg-zinc-800 border border-gray-300"
...
>
  <div className="relative">
    ...
    />
    <img
      className="w-10 h-10 rounded-full object-cover absolute top-1/2 left-2 transform -translate-y-1/2 border-2 border-white dark:border-zinc-800"
  ...
  <div className="p-3">
    <div className="mb-1">
      <span className="block text-lg font-semibold text-green-400 dark:text-green-400">
        <span className="mr-1">{motorbike.model.modelName}</span>
        <span>{motorbike.yearOfManufacture}</span>
      </span>
    </div>
    <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-1 inline-flex items-center space-x-2">
      <FontAwesomeIcon
        icon={faLocationDot}
        size="sm"
        style={{ color: "#0f0000" }}
    ...
    <hr className="my-1 border-gray-300 dark:border-zinc-600" />
